...
class MlSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        nome_categoria = self.pegar_categoria(response.url)
        ...
        for i in response.xpath('//li[contains(@class, "ui-search-layout__item")]'):
            ...
            items.append(item)
        self.salvar_json(nome_categoria, items)

    def pegar_categoria(self, url):
        for category, links in self.categorias.items():
            if url in links:
                return category
        return 'Outros'

    def salvar_json(self, category_name, items):
        os.makedirs('data', exist_ok=True)
        file_path = os.path.join('data', f'{category_name}.json')
