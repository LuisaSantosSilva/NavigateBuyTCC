  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        router.push('/editar-perfil'); // Redireciona para a página inicial
      } else {
        setMessage(response.data.message);
      }
    } catch (error) {
              ...
              <div className="w-full px-3 relative">
                ...
                />
                 <input
                  id="email"
                  type="email"
                  className={`py-3 sm:py-4 md:py-5 lg:py-5 pl-12 sm:pl-14 md:pl-16 lg:pl-20 pr-4 w-full text-base sm:text-lg md:text-xl lg:text-2xl rounded-2xl border
                     border-black focus:outline-none shadow-md transition duration-500 ease-in-out 
                     largeInputOnDesktop ${getInputClass('email')}`}
                  placeholder="Email"
                  required
                  value={email}
